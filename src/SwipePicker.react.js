...
const {subtract, add} = require('bens_utils').vectors;
const {useMouseHandler, useEnhancedReducer, mouseReducer} = require('./hooks');
...
const SwipePicker = (props) => {
  const {
    ...
    clickToSelect, frozenOnOneOption,
  ...
  const getOptionAtOffset = (options, offset) => {
    ...
    for (let i = 0; i < options.length; i++) {
      ...
      const {x, width} = optionElem.getBoundingClientRect()
  ...
  const [state, dispatch, getState] = useEnhancedReducer(
    (state, action) => ({...state, mouse: mouseReducer(state.mouse, action)}),
    {mouse: {}, selectedIndex, left: 0, prevLeft: 0},
  );
  ...
  useMouseHandler(
    id, {dispatch, getState},
    {
      mouseMove: (state, dispatch, pixel) => {
        ...
        dispatch({left: state.prevLeft + subtract(pixel, state.mouse.downPixel).x});
        if (onMouseMove) {
          onMouseMove(pixel);
        }
      },
      ...
      leftUp: (state, dispatch, pixel) => {
        ...
        if (
          clickToSelect && indexAtPixel != null &&
        ...
        if (
          indexAtPixel != null &&
          ((indexAtPixel == state.selectedIndex && nextSelectedIndex == state.selectedIndex) ||
            clickToSelect) &&
        ...
        dispatch({selectedIndex: nextSelectedIndex});
        if (onSelectIndex) {
  ...
  return (
    ...
    >
      ...
      >
        {options.map((o, i) => (
          <PickerOption key={id + "_option_" + o.label + "_" + i}
            ...
            style={{
              ...
              background:
                getOptionDistFromLeft(i, state.left) < getOptionWidth(i)
                  ? `linear-gradient(to right, transparent 0%, ${o.color ?? defaultColor} ${300 - getOptionDistFromLeft(i, state.left) / getOptionWidth(i) * 300}%)`
                  : (getOptionDistFromRight(i, state.left) < getOptionWidth(i)
                    ? `linear-gradient(to left, transparent 0%, ${o.color ?? defaultColor} ${300 - getOptionDistFromRight(i, state.left) / getOptionWidth(i) * 300}%)`
                    : o.color ?? defaultColor // NO-OP
                 ),
            }}
            isSelected={i == state.selectedIndex}
            sizeMult={!state.mouse.isLeftDown
              ? (i == state.selectedIndex ? maxSize : minSize)
              : Math.max(maxSize - Math.abs(getOptionDistFromCenter(i, state.left))/(width/2), minSize)
            }
            id={id + "_option_" + i}
          />
        ))}
...
const PickerOption = (props) => {
  const {
    style, isSelected, onClick, label,
    width: parentWidth, height: parentHeight,
    isCircular, id, sizeMult, color: backgroundColor,
  } = props;
