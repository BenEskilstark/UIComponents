...
const useHotKeyHandler = (pseudoStore, noWASD, dependencies) => {
  useEffect(() => {
    const {dispatch, getState} = pseudoStore;
    ...
    const keyFn = (ev, keyEventType, pressed) => {
      ...
      if (dir != null) {
        ...
        dispatch({type: 'SET_KEY_PRESS', key: dir, pressed});
        return;
      }
      if (ev.keyCode === 13) {
        ...
        dispatch({type: 'SET_KEY_PRESS', key: 'enter', pressed});
        return;
      }
      if (ev.keyCode === 32) {
        ...
        dispatch({type: 'SET_KEY_PRESS', key: 'space', pressed});
        return;
      }
      ...
      if (character != null) {
        ...
        dispatch({type: 'SET_KEY_PRESS', key: character, pressed});
      }
    }
